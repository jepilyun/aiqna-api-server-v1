...
// 타입 정의 개선
type TContentValidator<T> = (data: T) => { isValid: boolean; error?: string };
type TContentProcessor<T> = (data: T) => Promise<void>;

// 각 타입별 성공 응답 타입 정의
type TYouTubeSuccessResponse = {
  success: true;
  videoId: string;
  message: string;
  statusUrl: string;
...
type TInstagramSuccessResponse = {
  success: true;
  instagramUrl: string;
  message: string;
  statusUrl: string;
...
type TSuccessResponse =
  | TYouTubeSuccessResponse
  | TInstagramSuccessResponse
  | TBlogSuccessResponse
  | TTextSuccessResponse;
...
class ContentProcessingUtils {
  static async processContent<T>(
    res: Response,
    data: T,
    validator: TContentValidator<T>,
    processor: TContentProcessor<T>,
