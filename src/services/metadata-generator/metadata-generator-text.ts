...
export class MetadataGeneratorText {
  ...
  async generateMetadataFromText(
    ...
    let content = "";
    if (textData.content) {
      // 문자수 기준 간단 트렁케이션 (토큰 예산 고려 필요시 추후 토크나이저 적용)
    ...
    for (let i = 0; i < this.models.length; i++) {
      ...
      try {
        console.log(`🔄 Trying ${provider}/${model} for ${textData.hash_key.slice(0, 16)}...`);
        ...
        const { message, status, code } = extractError(error);
        console.warn(`⚠️ ${provider}/${model} failed for ${textData.hash_key.slice(0, 16)}...: ${message}`);
    ...
    console.error(`❌ All models failed for ${textData.hash_key.slice(0, 16)}..., returning empty metadata`);
  ...
  private getSystemPrompt(): string {
    // source: "text" 고정, 나머지는 옵션 사용
    return buildSystemPrompt({
      source: this.options.source ?? "text",
