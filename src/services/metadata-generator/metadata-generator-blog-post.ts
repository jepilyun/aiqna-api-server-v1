...
/**
 * Metadata Generator for Blog Post
 */
export class MetadataGeneratorBlogPost {
  ...
  async generateMetadataFromBlogPost(
    ...
    if (content.length === 0) {
      console.warn(`⚠️ No content available for ${blogPost.blog_post_url}`);
    ...
    for (let i = 0; i < this.models.length; i++) {
      ...
      try {
        console.log(`🔄 Trying ${provider}/${model} for ${blogPost.blog_post_url}`);
        ...
        const { message, status, code } = extractError(error);
        console.warn(`⚠️ ${provider}/${model} failed for ${blogPost.blog_post_url}: ${message}`);
    ...
    console.error(`❌ All models failed for ${blogPost.blog_post_url}, returning empty metadata`);
    return this.getEmptyMetadata(language);
  }
  ...
  private getSystemPrompt(): string {
    // source: "blog" 고정, 나머지는 옵션 사용
    return buildSystemPrompt({
      source: this.options.source ?? "blog",
  ...
  private buildPrompt(content: string, language: string): string {
    return buildUserPrompt({
      source: this.options.source ?? "blog",
      title: undefined, // 블로그 본문 분석이므로 별도 제목 불필요(있다면 넣어도 무방)
      language,
      body: content,
    });
  }
