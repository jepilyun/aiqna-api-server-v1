...
export function chunkYouTubeDescription(
  ...
  for (let i = 0; i < rawLines.length; i++) {
    let l = normalizeLine(rawLines[i], {
      stripUrls,
      stripHashtags,
      normalizeBullets,
    });
    if (cleanText) l = l.replace(/[ \u00A0]{2,}/g, " ").trim();
    if (dropShortLineUnder > 0 && l.trim().length < dropShortLineUnder)
      continue;
    lines.push(l);
  }
  ...
  for (let i = 0; i < lines.length; i++) {
    if (i > start && isBreakLine(i)) {
      ...
      if (slice)
        sections.push({ startLine: start, endLine: i - 1, text: slice });
      start = i + 1; // 경계라인 스킵
    }
  }
  ...
  if (!coalesceSmallSections) {
    ...
    for (let i = 1; i < sections.length; i++) {
      const s = sections[i];
      if (
        bucket.text.length < minSectionChars ||
        s.text.length < minSectionChars
      ) {
        bucket = {
  ...
  // 4) 예산/오버랩 계산
  const useTokens = !!tokenCounter && (maxTokens || minTokens || overlapTokens);
  const lengthOf = (t: string) => (useTokens ? tokenCounter!(t) : t.length);

  const maxBudget = useTokens
    ? (maxTokens ?? Math.round(maxChars / 4))
    : maxChars;
  ...
  const appendSection = (sec: Section) => {
    const secLines = Array.from(
      { length: sec.endLine - sec.startLine + 1 },
      (_, k) => sec.startLine + k,
    );
  ...
  if (verbose) {
    ...
    if (over.length) {
      ...
      over.forEach((c) =>
        console.warn(
          ` - #${c.index}: ${lengthOf(c.text)} ${useTokens ? "tokens" : "chars"}`,
        ),
      );
    }
  }
