...
/**
 * Instagram Post Metadata Extractor with fallback support
 */
export class InstagramPostMetadataAnalyzerByAI {
  ...
  async analyzeFromInstagramPost(
    ...
    if (content.length === 0) {
      console.warn(
        `⚠️ No content available for ${instagramPost.instagram_post_url}`
      );
      return {
        categories: [],
        keywords: [],
    ...
    for (let i = 0; i < this.models.length; i++) {
      ...
      try {
        console.log(
          `🔄 Trying ${provider}/${model} for ${instagramPost.instagram_post_url}`
        );
        ...
        const errorMsg = (error as Error).message;
        console.warn(
          `⚠️ ${provider}/${model} failed for ${instagramPost.instagram_post_url}: ${errorMsg}`
        );
    ...
    console.error(
      `❌ All models failed for ${instagramPost.instagram_post_url}, returning empty metadata`
    );
    return {
      categories: [],
      keywords: [],
  ...
  private getSystemPrompt(): string {
    return `You are an expert at analyzing Instagram content about Korean travel, food, and lifestyle.
