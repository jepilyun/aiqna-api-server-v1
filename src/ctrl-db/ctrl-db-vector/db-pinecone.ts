import { TPineconeMetadata, TPineconeQueryResult, TPineconeVector } from "aiqna_common_v1";
...
export default class DBPinecone {
  ...
  static async upsertBatch(
    ...
    try {
      ...
      console.log(`✓ Upserted ${vectors.length} vectors in ${totalBatches} batches`);
    } catch (error) {
  ...
  static async deleteAll(
    ...
    try {
      ...
      console.log(`✓ Deleted all vectors${namespace ? ` in namespace: ${namespace}` : ''}`);
    } catch (error) {
