...
export class HelperYouTube {
   ...
   */
  static buildWatchUrl(videoId: string, start?: number): string {
    const timeParam = start ? `&t=${Math.max(0, Math.floor(start))}s` : "";
...
export const extractYouTubeVideoId = HelperYouTube.extractVideoId;
